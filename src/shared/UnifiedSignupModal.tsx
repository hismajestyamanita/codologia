...
const UnifiedSignupModal: React.FC<UnifiedSignupModalProps> = ({
  ...
  const getAgeGroupFromProgram = (programTitle: string) => {
    const t = programTitle || '';
    // Новые названия программ
    if (/Программирование/.test(t)) {
      if (/(8–?10|8-10)/.test(t)) return 'Программирование (8–10 лет)';
      if (/(10–?12|10-12)/.test(t)) return 'Программирование (10–12 лет)';
      if (/(13\+|13\+)/.test(t)) return 'Программирование (13+ лет)';
    }
    // Прямое соответствие по полному названию
    const known = [
      'Программирование (8–10 лет)',
      'Программирование (10–12 лет)',
      'Программирование (13+ лет)',
    ...
    // Бэкап: старые названия на случай, если пришли из закешенных ссылок
    if (/Компьютерная грамотность и первые игры|8-10/.test(t)) return 'Программирование (8–10 лет)';
    if (/Мобильные приложения|10-12/.test(t)) return 'Программирование (10–12 лет)';
    if (/Профессиональная IT-разработка|13\+/.test(t)) return 'Программирование (13+ лет)';
    if (/Компьютерные каникулы/.test(t)) return 'Компьютерные каникулы';
    if (/Компьютерный дизайн/.test(t)) return 'Компьютерный дизайн';
    if (/Компьютерная грамотность$/.test(t)) return 'Компьютерная грамотность';
    return '';
  };
        ...
        <form onSubmit={handleSubmit} className="relative z-0 p-6 space-y-6">
          ...
          <div>
            ...
            >
              ...
              <option value="Программирование (8–10 лет)">Программирование (8–10 лет)</option>
              <option value="Программирование (10–12 лет)">Программирование (10–12 лет)</option>
              <option value="Программирование (13+ лет)">Программирование (13+ лет)</option>
